...
Важно:
- `result` хранит итоговый человекочитаемый текст результата, например: "Боль требует внимания" или "Рекомендуем обратиться к специалисту". Сценарий может использовать 2, 3, 4 и более уровней; `low/medium/high` — лишь один из возможных наборов названий уровней.
- Поддерживается взвешенный скоринг: `total = sum(option_score * question_weight)`. Поле `question_weight` необязательное: без него используется вес `1`, поэтому старые сценарии не меняют результат; для взвешенного сценария коэффициенты задаются явно, например `1, 10, 100, 1000...`. Вариант с `score: 0` завершает квиз на текущем шаге.
...
```html
<iframe
  src="https://painhelp.vercel.app/1st-pain-quiz-styled.html?quiz=pain-v1"
  width="100%"
  height="720"
  frameborder="0"
