...
### Редактировать существующий квиз
...
**Что происходит:**
1. Параметр `?quiz=pain-v1` указывает на файл `scenarios/pain-v1.json`
2. Параметр `&edit=Y` активирует режим редактирования
3. Квиз загружается обработчиком
4. Редирект на Quiz Editor с предзаполненными данными
5. Открывается интерфейс редактора с загруженным JSON

**После редактирования:**
- Скачайте обновленный JSON (кнопка «Экспорт JSON»)
- Замените старый файл в `scenarios/`
- Коммитьте и пушьте в GitHub
