...
- В новых сценариях используется взвешенный скоринг: `total = sum(option_score * question_weight)`, где `question_weight` по умолчанию растёт как `1, 10, 100, 1000...` по порядку вопросов; вариант с `score: 0` завершает квиз на текущем шаге.
