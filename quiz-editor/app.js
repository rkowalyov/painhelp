...
// ---------- THEME ----------
(function () {
  const THEME_KEY = 'quiz-editor-theme';
  ...
  const applyTheme = (nextTheme) => {
    ...
    if (t) t.innerHTML = nextTheme === 'dark' ? sunSVG : moonSVG;
    try {
  ...
  let d = getStoredTheme() || (matchMedia('(prefers-color-scheme:dark)').matches ? 'dark' : 'light');
  applyTheme(d);

  if (t) {
    t.addEventListener('click', () => {
      const nextTheme = d === 'dark' ? 'light' : 'dark';
      d = nextTheme;
...
function initFieldHelp() {
  ...
  const helpMap = {
    ...
    'crm-score': {
      title: 'Итоговый балл',
      text: 'Числовой итог score по всем ответам пользователя. Это число используется для выбора уровня результата по сортированным порогам в сценарии; уровни могут быть 2, 3, 4 и более.',
      target: 'data'
    },
    'crm-result': {
